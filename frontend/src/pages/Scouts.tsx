...
function Scouts() {
  ...
  useEffect(() => {
    api.scouts.list()
      .then(({ scouts: records }) => setScouts(records))
      .catch((loadError) => {
        setError(loadError instanceof Error ? loadError.message : "Could not load scouts.");
      })
            ...
            <tbody>
              {filteredScouts.map((scout) => (
                <tr key={scout.id}>
                  ...
                  <td>{scout.age}</td>
                  <td>{scout.phone}</td>
                  <td>{new Date(scout.joinedAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</td>
