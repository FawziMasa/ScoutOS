...
import { api, eventTypes, type EventType, type Scout, type ScoutEvent, type ScoutEventInput } from "../lib/api";
...
function Events() {
  ...
  return (
    <div className="page">
      <header className="page-header"><div><span className="eyebrow">Group calendar</span><h1>Events</h1><p>Plan meetings, adventures, and community projects with the right scouts registered.</p></div><button className="button button-primary" onClick={openCreate}><Icon name="plus" size={18} />Create event</button></header>
      ...
      <section className="panel table-panel">
        ...
        <div className="table-wrap events-table-wrap"><table><thead><tr><th>Event</th><th>Schedule</th><th>Location</th><th>Registrations</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{filteredEvents.map((event) => <tr key={event.id}><td><div className="event-title-cell"><span className="event-calendar-icon"><Icon name="events" size={17} /></span><div><strong>{event.title}</strong><span>{event.eventType}</span></div></div></td><td><strong>{formatDate(event.startDate)}{event.endDate !== event.startDate ? ` - ${formatDate(event.endDate)}` : ""}</strong><span className="event-subtle">{timeRange(event)}</span></td><td>{event.location || "Location to be confirmed"}</td><td><button className="registration-button" onClick={() => openRegistrations(event)}>{event.registrationCount}{event.capacity ? ` / ${event.capacity}` : ""} scouts</button></td><td><div className="table-actions"><button aria-label={`Edit ${event.title}`} onClick={() => openEdit(event)}><Icon name="edit" size={17} /></button><button className="danger" aria-label={`Delete ${event.title}`} onClick={() => deleteEvent(event)}><Icon name="trash" size={17} /></button></div></td></tr>)}</tbody></table>
          {loading && <div className="empty-state"><p>Loading events...</p></div>}
          {!loading && filteredEvents.length === 0 && <div className="empty-state"><span><Icon name={events.length === 0 ? "events" : "search"} size={25} /></span><h3>{events.length === 0 ? "No events planned yet" : "No events found"}</h3><p>{events.length === 0 ? "Create the first group event to start your calendar." : "Try a different search or date filter."}</p>{events.length === 0 && <button className="button button-primary empty-state-button" onClick={openCreate}><Icon name="plus" size={17} />Create first event</button>}</div>}
        </div>
      </section>
