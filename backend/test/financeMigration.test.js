...
test("Finance migration uses a text bridge, normalizes, then narrows legacy statuses", async () => {
  ...
  try {
    ...
    const widen = statements.findIndex((sql) => sql.includes("MODIFY COLUMN status VARCHAR(20) NULL"));
    const normalize = statements.findIndex((sql) => sql.includes("WHEN 'COMPLETED' THEN 'APPROVED'") && sql.includes("ELSE 'DRAFT'"));
    const narrow = statements.findIndex((sql, index) => index > widen && sql.includes("ENUM('DRAFT', 'SUBMITTED', 'APPROVED', 'REJECTED', 'CANCELLED')"));
    assert.ok(widen >= 0);
    assert.ok(normalize > widen);
    assert.ok(narrow > normalize);
...
test("Finance migration normalizes a legacy VARCHAR status before creating the ENUM", async () => {
  const originalExecute = db.execute;
  const calls = [];
  db.execute = async (sql, values = []) => {
    calls.push(String(sql).replace(/\s+/g, " ").trim());
    if (String(sql).includes("information_schema.columns")) {
      return [[{ column_name: values[1], column_type: values[1] === "status" ? "varchar(20)" : "int" }]];
    }
  ...
  try {
    await ensureFinanceSchema();
    const textBridge = calls.findIndex((sql) => sql.includes("MODIFY COLUMN status VARCHAR(20) NULL"));
    const normalize = calls.findIndex((sql) => sql.includes("WHEN 'PENDING' THEN 'SUBMITTED'"));
    const enumWrite = calls.findIndex((sql) => sql.includes("MODIFY COLUMN status ENUM('DRAFT'"));
    assert.ok(textBridge >= 0);
    assert.ok(normalize > textBridge);
    assert.ok(enumWrite > normalize);
  } finally {
    db.execute = originalExecute;
  }
});
